...
def max_pool(input_data, kernel_size = 5, stride = 1, padding = 2):
    # Add padding to the input data
    input_data_padded = torch.nn.functional.pad(input_data, (2, 2, 2, 2), value=0)
    input_data_padded = input_data_padded[0][0]
    # Calculate output shape
    output_height = (input_data_padded.shape[0] - 4 + 2 * padding - kernel_size) // stride + 1
    output_width = (input_data_padded.shape[1] - 4 + 2 * padding - kernel_size) // stride + 1
    ...
    for i in range(0, output_height):
        for j in range(0, output_width):
            ...
            # Extract the region from the padded input data
            region = input_data_padded[start_i:end_i, start_j:end_j]
            
            # Compute the maximum value in the region and store it in the output array
            output_data[i, j] = np.max(np.array(region))
...
def nms_fast(scores, H, W, dist_thresh = 3):
    """
...
class DKD(nn.Module):
    ...
    def detect_keypoints(self, scores_map, sub_pixel=True):
        ...
        nms_scores = nms_fast(scores_nograd, 2)
        end_time = time.time()
        execution_time = end_time - start_time
        print("Execution time(nms_time):", execution_time, "seconds")
        # remove border
